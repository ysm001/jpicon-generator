...
const paintColor = require('./paint-color.js');

module.exports = class Kaleidoscope {
  constructor(image, size, options = {}) {
    this._options = this._parseOptions(options);
    this._image = this._options.palette ? paintColor(image, this._options.palette) : image;
    this._radius = size / 2;
  ...
  generate() {
    ...
    const arcRadScale = this._options.shape === 'square' ? Math.sqrt(2) : 1;
    ...
    range.forEach(i => {
      ...
      this._context.arc(0, 0, this._radius * arcRadScale, step * -0.53, step * 0.53);
